...
This guide will use MaNGOS Three(Cataclysm). If you choose to use another version, you can simply just change the cloning URL to the version you would like.
...
If you want to use the development repository (that can contain errors and other stuff non fully working) you can clone the "develop" branc with this commands. I've supposed to have my local copy into *server-dev21* folder and use the *build-dev21* folder to compile and buil sources. Finally i place binaries into *run-dev21* folder.
    ...
    git clone -b develop21 -c http.proxy=http://127.0.0.1:3128 https://github.com/Corsol/database.git database-dev21
	cd database-dev21
	git clone --recursive https://github.com/mangos/Realm_DB.git Realm
    git clone --recursive -c http.proxy=http://127.0.0.1:3128 https://github.com/Corsol/Realm_DB.git Realm
	cd ..
	git clone -b develop21 https://github.com/mangosthree/server.git server-dev21
    git clone -b develop21 -c http.proxy=http://127.0.0.1:3128 https://github.com/Corsol/server.git server-dev21
    cd server-dev21
    git clone --recursive -b Rel21_StormlibForM3 https://github.com/mangos/mangosDeps.git dep
    git clone --recursive -b Rel21_StormlibForM3 -c http.proxy=http://127.0.0.1:3128 https://github.com/Corsol/mangosDeps.git dep
    cd src
    git clone --recursive -b Rel21_Revision https://github.com/mangos/realmd.git realmd
    git clone --recursive -b Rel21_Revision -c http.proxy=http://127.0.0.1:3128 https://github.com/Corsol/realmd.git realmd
    cd ../../..
	mkdir build-dev21
    mkdir run-dev21
    cd build-dev21`
...
###3.1.1. **OPTIONAL** ScripDev 3
    ...
    git clone -c http.proxy=http://127.0.0.1:3128 https://github.com/Corsol/ScriptDev3.git SD3`
To ensure the compilation and installation of this new library you need to modify some *CMakeLists.txt* files. Here a lists of location to look and edit:
  ...
  * /server-dev21/src/module/CMakeLists.txt: In this file you need to put this line code `if(SCRIPT_LIB_SD3)
    add_subdirectory(SD3)
endif()` after the *SCRIPT_LIB_SD2* include section
  * /server-dev21/src/mangosd/CMakeLists.txt: In this file you need to put this line code `if(SCRIPT_LIB_SD3)
...
git clone --recursive develop21 -c http.proxy=http://127.0.0.1:3128 https://github.com/mangosthree/database.git database-dev21
...
**NOTE**: If you wish to use submodules with your server, you can enable or disable them during the cmake step by adding the following before your PREFIX: (See details at [link](https://github.com/mangosthree/server/blob/develop21/CMakeLists.txt))
...
If you **do not** want to use the development repository (that can contain errors and other stuff non fully working) you can clone the "master" branc with this commands. I've supposed to have my local copy into *server* folder and use the *build* folder to compile and buil sources. Finally i place binaries into *run* folder.
	...
	git clone --recursive -c http.proxy=http://127.0.0.1:3128 https://github.com/Corsol/database.git database
	git clone --recursive https://github.com/mangosthree/server.git server
	git clone --recursive -c http.proxy=http://127.0.0.1:3128 https://github.com/Corsol/server.git server
	cd server/src/bindings
	git clone --recursive https://github.com/mangosthree/scripts.git
	git clone --recursive -c http.proxy=http://127.0.0.1:3128 https://github.com/Corsol/scripts.git
...
**NOTE**: If you wish to use submodules with your server, you can enable or disable them during the cmake step by adding the following before your PREFIX: (See details at [link](https://github.com/mangosthree/server/blob/master/CMakeLists.txt))
(0=disabled 1=enabled)
...
The PREFIX argument specifies where the server will be installed. Change it accordingly.
    ...
    cmake ../ -DCMAKE_INSTALL_PREFIX=/home/mangos/zero -DINCLUDE_BINDINGS_DIR=scripts
